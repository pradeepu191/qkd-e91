...
1. Confirm host/REST port/WS port match your setup. With the proxy
   running locally and the device tunneled to `localhost:8082`, the
   defaults work as-is.
2. Click **Connect WebSocket**. The status pill in the top-right
   should turn green.
3. Click **Push exposure to device** to set the integration window.
4. Click **Ping** to confirm coincidence counts arrive.
